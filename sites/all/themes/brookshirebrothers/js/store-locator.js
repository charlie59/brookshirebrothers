...
jQuery(document).ready(function () {
  ...
  function initFilterLocation() {
      ...
      if ("geolocation" in navigator) {
        ...
        navigator.geolocation.getCurrentPosition(function (position) {
              ...
              jQuery.getJSON('https://maps.googleapis.com/maps/api/geocode/json?latlng=' + latlng + '&result_type=street_address&key=' + google_maps_api_key, function (data) {
                var timer = setTimeout(stopLookup, 15000); // bail after 15
                                                           // seconds
                // console.log(data);
                  ...
                  if (data.results[0]) {
                    ...
                    for (j = 0; j < obj.address_components.length; j++) {
                      if (obj.address_components[j].types[0] === 'postal_code') {
                        ...
                        // obj.address_components[j].short_name);
              ...
              // alert(error);
              // alert('We were unable to determine your location. Please enter
              // a zip code.'); console.log(error);
              searchBox.removeClass('italic').val('');
            }
        );
      }
    ...
    jQuery('.filter-location').each(function () {
      ...
      function sendForm(e) {
        storeZip = searchBox.val(); // they might gone back to the form and
                                    // changed it from the cookie value
      ...
      function getCoordinates(obj, yourCoord, limit) {
        ...
        jQuery.each(semiresultsArray, function (ind, obj) {
          ...
          for (var i = 0; i < intermediateArr.length; i++) {
            var check;
            check = jQuery.trim(intermediateArr[i]);
            var a;
