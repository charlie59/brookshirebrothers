          ...
          <?php $node_week = node_load($resul->field_weekly_ad_association['und'][0]['nid']); ?>
            {
                ...
                features: [{
                    ...
                    keywords: "<?php if ((isset ($resul->field_department['und'])) && (!empty
                    ($resul->field_department['und']))): foreach($resul->field_department['und'] as $term_id): $term = taxonomy_term_load($term_id['tid']);?> <?php echo $term->name;?>,<?php endforeach; ?><?php foreach($resul->field_specification['und'] as $term_id): $term = taxonomy_term_load($term_id['tid']);?> <?php echo $term->name;?>,<?php endforeach; ?> <?php foreach($resul->field_locations['und'] as $term_id): $term = taxonomy_term_load($term_id['tid']);?> <?php echo $term->name;?>,<?php endforeach; ?> <?php endif; ?>"
                }]
